...
export const useRaceStore = create<RaceStore>((set, get) => ({
  ...
  loadFromGpx: async (file) => {
    ...
    const race: Race = { id, name: file.name.replace(/\.gpx$/i, ''), date: '', description: '', startTime: '', cpMultipliers: {}, cpBreakTimes: {} }
    const route: Route = {
  ...
  loadFromZip: async (file) => {
    ...
    set({
      race: { ...data.race, startTime: data.race.startTime ?? '', cpMultipliers: data.race.cpMultipliers ?? {}, cpBreakTimes: data.race.cpBreakTimes ?? {} },
      routes: data.routes,
